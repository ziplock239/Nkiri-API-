...
def parse_download_links(html: str) -> list[dict]:
    ...
    def add(url: str, label: str = "Download"):
        ...
        if not url or url in seen or url.startswith("javascript") or url == "#":
            return
        # CRITICAL: reject any link that points back to thenkiri.com.ng
        # Those are related posts / nav links, never download links
        parsed = urlparse(url)
        if "thenkiri.com.ng" in parsed.netloc:
            return
        seen.add(url)
        host = parsed.netloc.replace("www.", "")
        results.append({"label": label.strip() or "Download", "url": url, "host": host})

    # Scope to main content area only — ignore header, footer, sidebar, related posts
    content = (
        ...
        or soup  # last resort: whole page (filtered by add() above)
    )

    # 1. Anchor tags inside content
    for a in content.find_all("a", href=True):
        href = a["href"]
        text = a.get_text(strip=True)

        # Direct video file in URL
        if FILE_EXT_RE.search(href):
            add(href, text or "Direct Download")
            continue

        # Known file-host domain in URL
        if any(h in href for h in FILE_HOSTS):
            add(href, text or "Download")
            continue

        # Quality/resolution keywords in link text — but only for external links
        if re.search(r'\b(480p|720p|1080p|2160p|4k|bluray|webrip|hdcam|dvdrip)\b', text, re.I):
            if href.startswith("http"):
                add(href, text)

    # 2. Iframes embedding file hosts
    ...
    # 3. Direct video URLs buried in <script> tags
    for script in soup.find_all("script"):
        raw = script.string or ""
        for m in re.finditer(
            r'["\']((https?://[^"\']{10,}?\.(?:mp4|mkv|avi|webm))[^"\']*)["\']', raw, re.I
        ):
            add(m.group(1), "Direct Video")
        # Also catch file-host URLs in JS (e.g. var url = "https://mediafire.com/...")
        for m in re.finditer(r'["\']((https?://(?:' + '|'.join(re.escape(h) for h in FILE_HOSTS) + r')[^"\']+))["\']', raw):
            add(m.group(1), "Download")

    # 4. Meta refresh redirect to a download page
